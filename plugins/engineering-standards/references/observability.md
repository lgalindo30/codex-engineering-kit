...
Use Pino with JSON console output in production and readable console output in development. Support
...
observability framework. Verify the chosen Pino integration/transports against the pinned Bun version.
...
return [safe Problem Details](api-contracts.md#errors) to callers. Avoid duplicate access logs from
...
Check the exact SDK/instrumentation versions on Bun: Node support does not prove Bun compatibility.
